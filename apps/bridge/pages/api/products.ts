...
export default async function handler(
  ...
  // Always vary by Origin (important for CDN caching)
  res.setHeader("Vary", "Origin")

  // Enforce origin allowlist + set CORS
  if (origin) {
  ...
  // Require Origin (browser-only access)
  if (!origin) {
  ...
  try {
    ...
    const r = await fetch(
      `${baseUrl}/product?cursor=${encodeURIComponent(cursor)}`,
      {
        headers: {
    ...
    if (!r.ok) {
      ...
      return res.status(r.status).json({
        error: "Commerce7 request failed",
        details: text,
      })
    }
